...
async function importarTerritorios() {
  ...
  try {
    ...
    for (const row of candidatosData) {
      ...
      if (tipo && tipo.toUpperCase() === 'DIPUTADO' && territorio && region) {
    ...
    for (const [numeroSenatorial, comunas] of senatorialesComunasMap.entries()) {
      ...
      // Busca la región en los candidatos SENADORES
      let regionNombre = null;
      for (const row of candidatosData) {
        const territorio = row[CANDIDATOS_COL_TERRITORIO];
        const tipo = row[CANDIDATOS_COL_TIPO];
        const region = row[CANDIDATOS_COL_REGION];
        if (
          tipo && tipo.toUpperCase() === 'SENADOR' &&
          territorio && territorio === nombreCircunscripcion &&
          region
        ) {
          regionNombre = region;
          break;
      ...
      if (!regionNombre) {
        regionNombre = `Región Desconocida (${nombreCircunscripcion})`;
      }
